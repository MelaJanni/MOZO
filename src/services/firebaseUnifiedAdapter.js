...
class UnifiedWaiterNotifications {
    ...
    this.db = null; // Database instancia real
    ...
    async startListening() {
        ...
        if (!this.db) {
            try {
                this.db = await getRealtimeDB();
            } catch (e) {
                console.error('❌ No se pudo obtener instancia Realtime DB:', e);
                return;
            }
        }

        // 🎯 Escuchar índice de llamadas activas del mozo
        const waiterRef = ref(this.db, `waiters/${this.waiterId}/active_calls`);
    ...
    addCallListener(callId) {
        if (!this.db) return; // Aún no listo
        const callRef = ref(this.db, `active_calls/${callId}`);
...
export function initializeUnifiedWaiterNotifications(waiterId) {
    ...
    unifiedNotifications.startListening(); // async (no await para no bloquear UI)
